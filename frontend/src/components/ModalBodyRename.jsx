...
const ModalBodyRename = () => {
	...
	const name = useSelector((state) => state.channelsInfo.channels).filter((item) => item.id === id).map((item) => item.name)[0];
	...
	return (
		...
		>
			{({ values, errors, handleChange, isValid, handleSubmit, isSubmitting }) => (
				<form>
					<label className="visually-hidden" htmlFor="name">{t('modal.label.channel')}</label>
					<input
